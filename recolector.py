    ...
    for linea in lineas:
        if "|" in linea:
            ...
            if len(partes) >= 8:
                ...
                badge_clon = f'<span class="text-[10px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded border border-yellow-500/30 mt-2 inline-block font-bold">🗞️ Cubierto por {cantidad_diarios} medios</span>' if cantidad_diarios > 1 else ""
...
for activo in activos:
    try:
        ...
        if req_yahoo.status_code == 200:
            ...
            widgets_html += f"""
            <div class="bg-[#0f172a]/80 backdrop-blur-xl border border-gray-700/60 rounded-xl p-4 flex-1 min-w-[150px] shadow-[0_8px_30px_rgb(0,0,0,0.5)] hover:border-blue-500/50 transition">
                <span class="text-blue-400 text-[10px] font-black tracking-wider uppercase">{activo['nombre']}</span>
                <div class="text-2xl font-mono font-black text-white mt-1">{precio_form}</div>
        ...
        botonesSent.forEach(boton => {{
            boton.addEventListener('click', () => {{
                ...
                if(boton.getAttribute('data-sent') === 'TODOS') {{
                    ...
                    boton.className = 'btn-sent bg-gray-800/40 border border-gray-500/30 text-gray-400 text-left px-4 py-2.5 rounded-xl font-semibold text-xs transition';
                }}
